...
const index: React.FC<IHomeProps> = (props) => {
    ...
    return <CustomContainer>
        ...
        <div ref={sideRef} className={drawerOpen ? 'home-side-container':'home-side-container-hide'}>
            ...
            <Collapse in={state[0]}>
                {
                    menu0.map((item,index)=>(
                        <div className={'home-sub-menu-item'} key={index}>{item}</div>
                    ))
                }
            </Collapse>
        ...
        <div className='home-button-wrapper'>
            <div className='home-search-button'>
                <MyButton onClick={toggle.bind(null,'search')} variant='outlined'>search</MyButton>
                {isSearch&&<div className='home-search'>
                    <div className='home-search-left'>
                        <div className='home-search-title'>Product Search</div>
                        <div className='home-search-text-black'>Simple search to discover any product's data metrics across main-stream e-commerce platforms</div>
            ...
            <div className='home-ask-button'>
                <AskButton onClick={toggle.bind(null,'ask')} variant='contained'>Ask Metric</AskButton>
                {isAsk&&<div className='home-ask-metric'>
                    <div className='home-ask-metric-left'>
                        <div className='home-ask-metric-title'>Ask Metric</div>
                        <div className='home-ask-metric-text-black'>Chat with our AI powered assistant to get your needed data easier:</div>
                        <div className='home-ask-metric-text'>
                            1.It understand what you needed and only fetch the data you needed.
                            2.It will draw the data metric for any specific ask, easily for export usage</div>
                    </div>
        ...
        >
            <div className='home-modal-container'>
                <Icon onClick={closeModal} sx={{position:'absolute',top:'15px',right:'15px',cursor:'pointer'}}>close</Icon>
                <div className='home-modal-header'>
                    <Image className={'home-modal-header-icon'} src={fireIcon} alt={""} />
                    <div className={'home-modal-header-title'}>Stay Tuned!</div>
                </div>
                <div className={'home-modal-content'}>{`We are not officially launched yet,\nplease join our waitlist`}</div>
