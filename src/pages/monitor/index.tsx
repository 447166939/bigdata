...
const MonitorPage:React.FC<MonitorProps>=()=>{
    ...
    function getContent(){
        if(menu=='Product Metrics'){return <Product />}
        else if(menu=='Platform Metrics'){return <Platform />}
        else {
            return <Saved />
        }
    }
    return (
        <div className={styles.container}>
           <div className={styles.header}>
               ...
               <div className={styles.menus}>{menus.map((item,index)=>(
                   <div className={styles.menu} onClick={clickMenu.bind(null,item)} key={index}>{item}</div>
               ))}</div>
